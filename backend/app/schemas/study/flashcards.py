...
class FlashcardBase(BaseModel):
    ...
    # Noun-phrase concept name (e.g. 'PostgreSQL MVCC') — used for concept_mastery bridging.
    # Separate from front_text (a question) so the learning loop has a clean concept key.
    topic: Optional[str] = Field(
        default=None,
        max_length=200,
        description="Noun-phrase concept name for the learning signal bridge",
    )
...
class FlashcardUpdate(BaseModel):
    ...
    front_media_url: Optional[str] = Field(default=None, description="Front media URL")
    back_media_url: Optional[str] = Field(default=None, description="Back media URL")
    topic: Optional[str] = Field(default=None, max_length=200, description="Noun-phrase concept name")
