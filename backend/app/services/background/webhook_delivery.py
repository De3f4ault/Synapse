...
Handles asynchronous webhook HTTP POST delivery with automatic retries.
"""
...
from app.core.events.webhooks.sender import send_webhook, calculate_signature
...
def deliver_webhook_task(
    ...
    if not result["success"] and self.request.retries >= self.max_retries:
        asyncio.run(_move_to_dlq(webhook_id, event_id))
        logger.error(
...
async def _deliver_webhook_async(
    ...
    """
    Internal async function to handle webhook delivery with database updates.
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Send webhook
        try:
            delivery_result = await send_webhook(
                webhook_url=webhook.url,
                event=event,
                secret=secret,
            ...
            webhook_event.status = WebhookStatus.SENT if delivery_result["success"] else WebhookStatus.FAILED
            webhook_event.response_code = delivery_result.get("status_code")
            webhook_event.response_body = delivery_result.get("response_body", "")[:1000]  # Limit size
            ...
            if delivery_result["success"]:
                ...
                if attempt_number < settings.WEBHOOK_MAX_RETRIES:
                    ...
                    webhook_event.next_retry_at = datetime.utcnow() + timedelta(seconds=backoff_seconds)
...
async def _move_to_dlq(webhook_id: int, event_id: str):
    """
