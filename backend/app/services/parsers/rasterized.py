...
class RasterizedDocumentParser(BaseDocumentParser):
    ...
    def parse(self, file_path: str, mime_type: str, filename: str) -> None:
        ...
        with OcrProcessor() as processor:
            ...
            # It lives inside OcrProcessor's tempdir which is cleaned up
            # when the `with` block exits — so copy it out first.
            raw_output = result.get("output_pdf")
            if raw_output and result.get("ocr_performed"):
                import os
                if os.path.exists(raw_output):
                    tmp = tempfile.NamedTemporaryFile(
                        delete=False, suffix=".pdf", prefix="synapse_archive_"
                    )
                    tmp.close()
                    shutil.copy2(raw_output, tmp.name)
                    self._archive_path = tmp.name
                else:
