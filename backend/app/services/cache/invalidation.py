...
class CacheInvalidator:
    ...
    def invalidate_document_cache(self, document_id: str) -> int:
        ...
        for pattern in patterns:
            keys = self.client.keys(pattern)
            if keys:
                total_invalidated += self.client.delete(*keys)
    ...
    def invalidate_by_tags(self, tags: Set[str]) -> int:
        ...
        for tag in tags:
            ...
            tagged_keys = self.client.get(tag_key, default=set())
            ...
            if tagged_keys:
                ...
                count = self.client.delete(*tagged_keys)
    ...
    def add_tag_to_key(self, key: str, tag: str) -> bool:
        """
        ...
        # Store updated tag
        return self.client.set(tag_key, list(tagged_keys))

    def invalidate_on_update(
        self,
        entity_type: str,
        entity_id: str,
        related_entities: Optional[List[tuple[str, str]]] = None
    ) -> int:
        """
        ...
        logger.info(
            f"Invalidated {total_invalidated} keys for {entity_type}:{entity_id}"
        )
        return total_invalidated

    def invalidate_on_delete(
        self,
        entity_type: str,
        entity_id: str
    ) -> int:
        ...
        logger.info(
            f"Invalidated {total_invalidated} keys on deletion of "
            f"{entity_type}:{entity_id}"
        )
        return total_invalidated

    def smart_invalidation(
        self,
        ...
        logger.info(
            f"Smart invalidation: {total_invalidated} keys for "
    ...
    def schedule_invalidation(
        ...
        return self.client.expire(key, delay_seconds)
