    ...
    try:
        ...
        if row:
            ...
            return DashboardOverview(
                ...
                study_streak_days=row["study_streak_days"],
...
async def get_performance(
    ...
    study_time_query = (
        ...
        .where(and_(StudySession.user_id == current_user.id, StudySession.started_at >= start_date))
        .group_by(func.date(StudySession.started_at))
    )

    study_time_result = await db.execute(study_time_query)
    study_time_by_date = {
        date: total_seconds // 60 for date, total_seconds in study_time_result.all()
    }
