...
CREATE OR REPLACE FUNCTION developer_schema.record_review(
        ...
        p_user_id INT,
        -- ID of the user performing review
        p_quality INT -- Quality of response (0-5)
    ) RETURNS JSONB LANGUAGE plpgsql VOLATILE -- Function modifies data
    AS $$
...
v_review_id INT;
v_result JSONB;
BEGIN -- Validate quality rating
IF p_quality < 0
OR p_quality > 5 THEN RETURN jsonb_build_object(
    'success',
    false,
    'error',
...
IF NOT FOUND THEN RETURN jsonb_build_object(
    ...
    false,
    'error',
    'Flashcard not found or has been deleted',
    'error_code',
    'CARD_NOT_FOUND'
);
END IF;
...
IF NOT EXISTS (
    ...
    false,
    'error',
    'User does not own this flashcard',
    'error_code',
    'UNAUTHORIZED'
);
END IF;
...
VALUES (
        ...
        0,
        -- Default time_taken_ms (can be updated by frontend later)
        NOW()
    )
RETURNING id INTO v_review_id;
-- Update flashcard with new values
UPDATE developer_schema.flashcards
SET ease_factor = v_sm2_result.new_ease_factor,
    ...
    learning_state = CASE
        ...
        WHEN v_sm2_result.new_interval >= 21
        AND v_sm2_result.new_ease_factor >= 2.5 THEN 'mastered'
...
COMMENT ON FUNCTION developer_schema.record_review(INT, INT, INT) IS 'Records a flashcard review atomically.
...
GRANT EXECUTE ON FUNCTION developer_schema.record_review(INT, INT, INT) TO synapse_user;