...
class CreateLinkTool(BaseTool):
    ...
    async def execute(self, user_id: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            from app.models.link import EntityType, LinkType
            from app.services.link_service import LinkService
            from app.db.session import get_async_session

            source_type = EntityType(kwargs.get("source_type"))
            source_id = kwargs.get("source_id")
            target_type = EntityType(kwargs.get("target_type"))
...
class GetKnowledgeGraphTool(BaseTool):
    ...
    async def execute(self, user_id: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            entity_types = None
            if entity_types_str:
                entity_types = [EntityType(t) for t in entity_types_str]
...
class SuggestLinksTool(BaseTool):
    ...
    async def execute(self, user_id: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            entity_type = EntityType(kwargs.get("entity_type"))
            entity_id = kwargs.get("entity_id")
            ...
            return {
                "success": False,
                "data": {},
                "message": f"Failed to suggest links: {str(e)}"
            }
