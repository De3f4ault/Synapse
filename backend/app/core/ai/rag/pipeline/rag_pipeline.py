...
class RAGPipeline:
    ...
    def __init__(
        ...
        self.enable_advanced_chunking = enable_advanced_chunking if enable_advanced_chunking is not None else False
        ...
        self.enable_reranking = enable_reranking if enable_reranking is not None else config.enable_cross_encoder
        self.reranker = None
        ...
        self.enable_learning_aware = enable_learning_aware if enable_learning_aware is not None else False
        ...
        self.enable_query_enhancement = enable_query_enhancement if enable_query_enhancement is not None else False
        ...
        self.enable_llm_enhancement = enable_llm_enhancement if enable_llm_enhancement is not None else False
        self.llm_enhancement_strategy = llm_enhancement_strategy
        self.llm_expander = None
        if self.enable_llm_enhancement:
            try:
                self.llm_expander = get_llm_expander(
        ...
        self.enable_feedback_loops = enable_feedback_loops if enable_feedback_loops is not None else False
    ...
    async def ingest_document(
        ...
        logger.debug("chunking_complete", chunk_count=len(chunks), method="advanced" if self.enable_advanced_chunking else "simple")
        ...
        collection_name = self.collection_manager.create_user_collection(
            user_id,
            source_type
        )
        
        # 5. Upsert to Qdrant
        # Generate IDs: {document_id}_{chunk_index}
    ...
    async def query(
        ...
        if self.enable_query_enhancement:
            ...
            logger.debug("query_expanded", original_len=len(original_query), expanded_len=len(query))
        ...
        if self.enable_llm_enhancement and self.llm_expander:
            try:
                ...
                strategy = strategy_map.get(self.llm_enhancement_strategy, EnhancementStrategy.REWRITE)
