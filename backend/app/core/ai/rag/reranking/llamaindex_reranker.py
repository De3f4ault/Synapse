...
# CrossEncoderReranker is imported locally in __init__ via get_cross_encoder_reranker
...
class CrossEncoderNodeReranker(BaseNodePostprocessor):
    ...
    def _postprocess_nodes(
        ...
        logger.info("reranking_nodes", query=query[:50], candidates=len(nodes), top_k=self.top_k)
        ...
        ranked_indices_scores = self._reranker.rerank(
            query=query, documents=documents, top_k=self.top_k
        )
