    ...
    def _check_keywords(self, message_lower: str) -> Optional[IntentClassification]:
        ...
        for intent, keywords in self.INTENT_KEYWORDS.items():
            ...
            if matches >= 2:
                ...
                return IntentClassification(
                    intent=intent, confidence=0.75, reasoning=f"Keyword match for {intent.value}"
                )
        return None
...
{{"intent": "tutor|document|quiz|workflow", "confidence": 0.0-1.0, "reasoning": "brief explanation"}}"""
        ...
        response = await llm.generate(prompt=prompt, model=DEFAULT_CHAT_MODEL, temperature=0.0)
