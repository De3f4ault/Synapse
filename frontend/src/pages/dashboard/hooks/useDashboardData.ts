...
export function useDashboardData() {
    ...
    const weakAreasQuery = useQuery({
        queryKey: queryKeys.analytics.weakAreas(),
                                    queryFn: () => getWeakAreasApiV1AnalyticsWeakAreasGet({ limit: 10 }),
    ...
    const performanceQuery = useQuery({
        queryKey: queryKeys.analytics.performance(30),
                                      queryFn: () => getPerformanceApiV1AnalyticsPerformanceGet({ days: 30 }),
    ...
    const heatmapQuery = useQuery({
        queryKey: queryKeys.analytics.heatmap(365),
                                  queryFn: () => getHeatmapApiV1AnalyticsHeatmapGet({ days: 365 }),
    ...
    const dueCardsQuery = useQuery({
        queryKey: queryKeys.flashcards.due(),
                                   queryFn: () => getDueCardsApiV1CardsDueGet({ limit: 50 }),
    ...
    const chatSessionsQuery = useQuery({
        queryKey: queryKeys.chat.sessions(),
                                       queryFn: () => listSessionsApiV1ChatSessionsGet({ pageSize: 50 }),
                                       staleTime: 1000 * 60 * 5,
    });
    ...
    const quizzesQuery = useQuery({
        queryKey: queryKeys.quizzes.lists(),
                                  queryFn: () => listQuizzesApiV1QuizzesGet({ pageSize: 50 }),
                                  staleTime: 1000 * 60 * 5,
    });
