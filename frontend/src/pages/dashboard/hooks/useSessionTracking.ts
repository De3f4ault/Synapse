...
export function useSessionTracking() {
    ...
    const [currentSession, setCurrentSession] = useState<DetectedSession | null>(null);
        ...
        switch (message.type || message.event) {
            ...
            case 'activity_history':
                ...
                if (message.data?.activities) {
                    ...
                    console.log('[Session Tracking] Loaded activity history:', activities.length);
                }
                break;
            ...
            case 'activity_logged':
                ...
                if (message.data) {
                    ...
                    console.log('[Session Tracking] Activity logged:', newActivity.activity_type);
                }
                break;
            ...
            case 'sessions_detected':
                ...
                if (message.data?.sessions) {
                    ...
                    console.log('[Session Tracking] Sessions detected:', detectedSessions.length);
                }
                break;
        }
    }, []);
    ...
    const logActivity = useCallback(
        (activity: Omit<ActivityLogEntry, 'id' | 'timestamp'>) => {
            ...
            console.log('[Session Tracking] Logging activity:', activity.activity_type, activity.module);
    ...
    const detectAndUpdateSessions = useCallback(() => {
        ...
        console.log('[Session Tracking] Local sessions detected:', detectedSessions.length);
    }, [activityLog]);
    ...
    const requestSessionDetection = useCallback(() => {
        if (!manager || !isConnected) {
            console.warn('[Session Tracking] Cannot request detection - not connected');
            return;
        }
