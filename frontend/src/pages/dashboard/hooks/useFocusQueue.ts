...
export function useFocusQueue(data: DashboardData | undefined) {
  ...
  const allQueueItems = useMemo((): QueueItem[] => {
    ...
    if (data.documents && Array.isArray(data.documents)) {
      data.documents
        .filter(
          (doc) =>
            doc.processing_status === "completed" && !doc.gemini_file_uri,
        )
        .forEach((doc) => {
          const priority = calculatePriority({
            dueDate: null,
  ...
  const sections = useMemo(() => {
    ...
    const highPriority = allQueueItems.filter(
      (item) => item.priority > 0.7 && !dueToday.some((d) => d.id === item.id),
    );

    // Recommended section (priority 0.4-0.7)
    const recommended = allQueueItems.filter(
      (item) =>
        item.priority >= 0.4 &&
        item.priority <= 0.7 &&
    ...
    const later = allQueueItems.filter(
      (item) =>
        item.priority < 0.4 &&
        !dueToday.some((d) => d.id === item.id) &&
        !highPriority.some((h) => h.id === item.id) &&
  ...
  const stats = useMemo(() => {
    return {
      ...
      highPriority:
        sections.find((s) => s.id === "high-priority")?.items.length || 0,
      estimatedTotalMinutes: allQueueItems.reduce(
        (sum, item) => sum + (item.estimatedMinutes || 0),
        0,
      ),
      byModule: {
        flashcards: allQueueItems.filter((i) => i.moduleType === "flashcards")
          .length,
        notes: allQueueItems.filter((i) => i.moduleType === "notes").length,
        documents: allQueueItems.filter((i) => i.moduleType === "documents")
          .length,
