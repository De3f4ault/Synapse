...
export function useIntelligence(data: DashboardData | undefined) {
  ...
  const weakAreas = useMemo((): WeakAreaInsight[] => {
    ...
    if (!data?.weakAreas || !Array.isArray(data.weakAreas)) {
      console.warn(
        "[useIntelligence] Invalid weakAreas data:",
        data?.weakAreas,
      );
      return [];
    }
    ...
    const unreadDocs =
      data.documents && Array.isArray(data.documents)
        ? data.documents.filter(
            (doc) => doc.processing_status === "completed" && !doc.user_id, // Simple heuristic: no user_id means unprocessed
          )
        : [];
...
function generateWeakAreaSuggestion(
  area: any,
