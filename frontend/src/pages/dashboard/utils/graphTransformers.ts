...
export function transformDocumentsToNodes(
  documents: DocumentResponse[] | undefined | null,
): GraphNode[] {
...
export function transformNotesToNodes(
  notes: NoteResponse[] | undefined | null,
): GraphNode[] {
...
export function transformFlashcardsToNodes(
  cards: FlashcardResponse[] | undefined | null,
): GraphNode[] {
  ...
  return cards.map((card) => {
    ...
    const label =
      frontText.substring(0, 30) + (frontText.length > 30 ? "..." : "");
...
export function transformChatsToNodes(
  chats: ChatSessionResponse[] | undefined | null,
): GraphNode[] {
...
export function transformQuizzesToNodes(
  quizzes: QuizResponse[] | undefined | null,
): GraphNode[] {
...
export function inferConnections(
  ...
  if (
    ...
    data.notes.forEach((note) => {
      data.documents?.forEach((doc) => {
        ...
        if (
          noteTitle.includes(docFilenameBase) ||
          noteContent.includes(docFilenameBase)
        ) {
          links.push({
  ...
  if (
    ...
    data.dueCards.forEach((card) => {
      ...
      data.notes?.forEach((note) => {
        ...
        if (
          cardText.length > 10 &&
          (noteContentLower.includes(cardText) ||
            noteTitleLower.includes(cardText))
        ) {
          links.push({
  ...
  if (data.dueCards && Array.isArray(data.dueCards)) {
    const cardsByDeck = data.dueCards.reduce(
      (acc, card) => {
...
function calculateNodeSize(type: string, metrics: Record<string, any>): number {
  const baseSize = 8;
  const maxSize = 24;
      ...
      const difficultyScore =
        metrics.difficulty === "hard"
          ...
          : metrics.difficulty === "medium"
            ...
            : 0.1;
      return (
        baseSize + (maxSize - baseSize) * (questionScore + difficultyScore)
      );
