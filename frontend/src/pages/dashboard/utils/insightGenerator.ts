...
function generateProgressInsights(
    ...
    if (cards_reviewed_today > 0) {
        const percentage = Math.round((cards_reviewed_today / (data.overview.due_cards || 1)) * 100);
...
function generateMasteryInsights(
    ...
    const criticalAreas = data.weakAreas.filter((area) => area.severity === 'critical');
            ...
            actions: [
                {
                    ...
                    metadata: { topic: criticalAreas[0].topic },
                },
            ],
        });
    }
            ...
            actions: [
                {
                    ...
                    action: 'review_weak_area',
                    metadata: { topic: improvableAreas[0].topic },
                },
            ],
        });
    }
...
function generateBehavioralSuggestions(
    ...
    const cardToDeckRatio = data.overview.total_cards / Math.max(data.overview.total_decks, 1);
    ...
    if (activityStats && activityStats.currentStreak < 3 && activityStats.totalActivities > 10) {
        insights.push({
