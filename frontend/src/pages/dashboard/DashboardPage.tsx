...
export function DashboardPage() {
    ...
    if (isLoading) return <div className="h-screen flex items-center justify-center bg-[#020202]"><LoadingState /></div>;
    if (error) return <div className="h-screen flex items-center justify-center bg-[#020202] text-red-500 font-mono">SYSTEM ERROR</div>;

    return (
        <div className="h-screen bg-[#020202] text-slate-200 flex flex-col overflow-hidden">

        {/* Background */}
        <div className="fixed inset-0 bg-gradient-to-b from-[#0c1214] via-[#020202] to-black">
    ...
    return (
        ...
        {nav.map(({ id, icon: Icon, color }) => (
            ...
            className={`p-3 rounded-full transition-all border ${
                activeView === id ? `${colors[color]} scale-110` : 'border-transparent hover:bg-white/5 text-slate-500'
            }`}
            >
