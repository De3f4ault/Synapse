...
function getTrendInfo(trendValue: number) {
    if (trendValue > 0) return { icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-500/10' };
    if (trendValue < 0) return { icon: TrendingDown, color: 'text-red-400', bg: 'bg-red-500/10' };
...
interface CommonStatProps { value: number; trend?: any; sparkline?: number[]; className?: string; onClick?: () => void; }

export function TotalReviewsCard({ value, trend, className, onClick }: CommonStatProps) {
    return <StatCard label="Total Reviews" value={value} formatter={(v) => v.toLocaleString()} trend={trend} icon={TrendingUp} iconColor="text-cyan-400" className={className} onClick={onClick} />;
}

export function AccuracyCard({ value, trend, className, onClick }: CommonStatProps) {
    return <StatCard label="Accuracy" value={value} suffix="%" decimals={1} trend={trend} icon={TrendingUp} iconColor="text-emerald-400" className={className} onClick={onClick} />;
}

export function StudyTimeCard({ value, trend, className, onClick }: CommonStatProps) {
    return <StatCard label="Study Time" value={value / 60} suffix="h" decimals={1} trend={trend} icon={TrendingUp} iconColor="text-purple-400" className={className} onClick={onClick} />;
}
