...
export function IntelligencePanel({ data }: IntelligencePanelProps) {
    ...
    return (
        ...
        {/* Empty State */}
        {!nextAction && weakAreas.length === 0 && milestones.length === 0 && contextInsights.length === 0 && (
            <div className="text-center py-12">
            <Brain className="w-16 h-16 mx-auto text-slate-600 mb-4" />
            <p className="text-slate-400">No insights available yet</p>
            <p className="text-sm text-slate-500 mt-2">Start studying to generate insights</p>
            </div>
        )}
        </div>
        </GlassCard>
