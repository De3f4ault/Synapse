...
export function MasteryOverview({ data }: MasteryOverviewProps) {
  ...
  return (
    ...
    {deckMastery.map((deck, index) => (
      ...
      {deck.cardCount} cards • {(deck.averageAccuracy * 100).toFixed(0)}% accuracy
      </span>
      <span className={cn(
        "font-semibold",
...
function calculateDeckMastery(data: DashboardData) {
  ...
  return Array.from(deckMap.values()).map(deck => {
    const masteredCount = deck.cards.filter(c => c.learning_state === 'mastered').length;
    const totalCards = deck.cards.length;
    const masteryScore = totalCards > 0 ? masteredCount / totalCards : 0;

    const averageAccuracy = deck.cards.reduce((sum, c) => {
...
function getMasteryBadgeColor(score: number): string {
  if (score >= 0.9) return 'bg-purple-500/20 text-purple-400 border-purple-500/30';
