...
import { BookOpen, FileText, ClipboardList, CheckCircle, Layers } from 'lucide-react';
...
export const ActionResultCard: React.FC<ActionResultCardProps> = ({ action, className }) => {
    const getActionIcon = () => {
        switch (action.type) {
    ...
    return (
        <Card className={cn(
            ...
            <CardContent className="space-y-2">
                {/* Flashcard Display */}
                {(action.type === 'create_flashcard' || action.type === 'update_flashcard') && (
                    <div className="space-y-1 text-sm">
                        <div className="flex items-start gap-2">
                            <span className="font-medium text-muted-foreground min-w-[60px]">Front:</span>
                            <span className="text-foreground">{action.data.front_text}</span>
                        </div>
                        <div className="flex items-start gap-2">
                            <span className="font-medium text-muted-foreground min-w-[60px]">Back:</span>
...
export const ActionsList: React.FC<ActionsListProps> = ({ actions, className }) => {
