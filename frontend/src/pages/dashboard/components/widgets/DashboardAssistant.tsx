...
export const DashboardAssistant: React.FC<DashboardAssistantProps> = ({
  ...
  const loadChatHistory = async (sessionId: number) => {
    ...
    try {
      const history =
        await ChatService.getMessagesApiV1ChatSessionsSessionIdMessagesGet(
          sessionId,
          50,
        );
  ...
  const loadSessions = async () => {
    try {
      const allSessions = await ChatService.listSessionsApiV1ChatSessionsGet(
        1,
        20,
      );
  ...
  const deleteSession = async (
    ...
    try {
      await ChatService.deleteSessionApiV1ChatSessionsSessionIdDelete(
        sessionIdToDelete,
      );
    ...
    if (!token) {
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "You need to be logged in to use the dashboard assistant. Please log in first.",
...
Title:`,
      ...
      if (response.ok) {
        ...
        if (!title || title.length > 60) {
          ...
          title =
            words.join(" ") + (firstMessage.split(" ").length > 5 ? "..." : "");
        }
              ...
              <div className="p-4 bg-[#0a0a0f] border-t border-white/5 relative z-10">
                ...
                >
                  ...
                  <NeumorphicButton
                    ...
                    variant="ghost"
                    disabled={
                      isTyping || isInitializing || !sessionId || !input.trim()
                    }
                  >
                    {isTyping ? (
