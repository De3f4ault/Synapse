      ...
      <div className="relative z-10 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <div
              className={cn(
                ...
                priorityColor.iconBg,
                priorityColor.iconBorder,
              )}
            >
        ...
        <div className="grid grid-cols-3 gap-2 py-2 border-y border-white/5 bg-black/20 rounded-lg px-2">
          <StatItem
            icon={Target}
            label="Priority"
            value={`${(recommendation.priority * 100).toFixed(0)}%`}
            color={priorityColor.text}
          />
          <StatItem
            icon={Clock}
            label="Est. Time"
            value={formatStudyTime(recommendation.estimatedMinutes)}
          />
          <StatItem
            icon={Zap}
            label="Confidence"
        ...
        <Button
          size="sm"
          className={cn(
            "w-full h-8 text-xs font-bold uppercase tracking-wide",
...
const StatItem = ({ icon: Icon, label, value, color }: any) => (
  <div className="flex flex-col items-center justify-center text-center">
    ...
    <div
      className={cn("text-xs font-mono font-bold", color || "text-slate-300")}
    >
      {value}
    </div>
  </div>
