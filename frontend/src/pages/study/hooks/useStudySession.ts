...
export function useStudySession(items: StudyItem[]): UseStudySessionReturn {
  ...
  const startSessionMutation = useMutation({
    mutationFn: () =>
      StudyService.startSessionApiV1StudySessionsPost({
  ...
  const completeSessionMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({
        queryKey: QUERY_KEYS.STUDY.RECOMMENDATIONS,
      });
    },
  });
  ...
  const handleAnswer = useCallback(
    (isCorrect: boolean) => {
      setSession((prev) => {
        ...
        const correctCount = Array.from(newCompletedItems.values()).filter(
          Boolean,
        ).length;
        const accuracy =
          completedCount > 0 ? (correctCount / completedCount) * 100 : 0;
