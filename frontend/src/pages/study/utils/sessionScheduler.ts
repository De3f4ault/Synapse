...
export function calculateSessionDuration(items: StudyItem[]): number {
    ...
    for (const item of items) {
        if (item.estimatedTime) {
            ...
            else if (item.type === 'quiz') totalTime += item.questionCount ? item.questionCount * 2 : 10;
            else totalTime += 5;
        }
    }
...
export function suggestOptimalStudyTime(): string {
    ...
    if (hour >= 6 && hour < 9) return 'Early morning - great for memory retention!';
    if (hour >= 9 && hour < 12) return 'Late morning - peak cognitive performance!';
...
export function createStudyChunks(totalMinutes: number): Array<{ type: 'study' | 'break'; duration: number }> {
    ...
    while (remaining > 0) {
        ...
        if (remaining > 0) {
            const breakDuration = chunks.filter(c => c.type === 'study').length % 4 === 0 ? 15 : 5;
