...
export function PriorityBar({ priority }: { priority: StudyPriority }) {
  ...
  return (
    <div className="flex items-end gap-0.5 h-4">
      <div
        className={`w-1 ${heights[priority]} ${colors[priority]} rounded-t`}
      />
      <div
        className={`w-1 ${priority === "high" ? heights.high : "h-1/2"} ${priority === "high" ? colors.high : "bg-gray-300"} rounded-t`}
      />
      <div
        className={`w-1 ${priority === "high" ? heights.high : "h-1/4"} ${priority === "high" ? colors.high : "bg-gray-300"} rounded-t`}
      />
    </div>
  );
}
