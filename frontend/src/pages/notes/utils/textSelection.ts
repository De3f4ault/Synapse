 ...
 */
export function getTextAreaSelection(element: HTMLTextAreaElement): TextAreaSelection {
    return {
 ...
 */
export function replaceCurrentLine(element: HTMLTextAreaElement, newText: string): void {
    const line = getCurrentLine(element);
    const value = element.value;

    element.value =
    value.substring(0, line.start) +
    newText +
    value.substring(line.end);
