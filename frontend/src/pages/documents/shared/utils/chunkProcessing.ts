 ...
 */
export function getChunkMetadata(chunk: ChunkMetadata): Record<string, unknown> {
    return {
        id: chunk.id,
...
export function mergeChunks(chunks: ChunkMetadata[]): string {
    ...
    const sortedChunks = [...chunks].sort((a, b) => {
        const indexA = (a.metadata as Record<string, number>)?.chunkIndex || 0;
        const indexB = (b.metadata as Record<string, number>)?.chunkIndex || 0;
