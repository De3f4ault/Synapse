 ...
 */
export function groupCardsByState(
  cards: Flashcard[],
): Record<LearningState, Flashcard[]> {
  return cards.reduce(
    (acc, card) => {
...
export function calculateSessionStats(
  ...
  const avgAccuracy =
    cards.reduce((sum, c) => sum + (c.accuracy ?? 0), 0) / totalCards;
  const avgInterval =
    cards.reduce((sum, c) => sum + (c.interval ?? 0), 0) / totalCards;
  ...
  const stateBreakdown = cards.reduce(
    ...
    { new: 0, learning: 0, review: 0, mastered: 0 } as Record<
      LearningState,
      number
    >,
  );
...
export function getNextReviewTime(cards: Flashcard[]): string | null {
  ...
  const diffHours = Math.round(
    (nextDate.getTime() - now.getTime()) / (1000 * 60 * 60),
  );
  ...
  return {
    ...
    averageAccuracy:
      recentCards.length > 0
        ? recentCards.reduce((sum, c) => sum + (c.accuracy ?? 0), 0) /
          recentCards.length
        : 0,
    newCardsMastered: recentCards.filter((c) => c.learning_state === "mastered")
      .length,
  };
}
