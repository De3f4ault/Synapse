...
export function useDeckStats(deckId: number) {
  ...
  if (cards && cards.length > 0) {
    ...
    const totalAccuracy = cards.reduce(
      (sum, card) => sum + (card.accuracy || 0),
      0,
    );
    ...
    const masteryPercent =
      totalCards > 0
        ? Math.round((stateGroups.mastered / totalCards) * 100)
        : 0;
  ...
  return {
    ...
    masteryPercent:
      Math.round((estimatedDistribution.mastered / totalCards) * 100) || 0,
