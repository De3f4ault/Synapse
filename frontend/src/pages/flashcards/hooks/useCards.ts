...
export function useDeckCards(deckId: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response =
        await FlashcardsService.getDueCardsApiV1CardsDueGet(deckId);
...
export function useCard(cardId: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response =
        await FlashcardsService.getCardApiV1CardsCardIdGet(cardId);
      return (response as any).data ?? response;
    },
    enabled: !!cardId,
  });
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.decks.cards(variables.deck_id),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.decks.detail(variables.deck_id),
      });
...
export function useUpdateCard() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await FlashcardsService.updateCardApiV1CardsCardIdPut(
        cardId,
        data,
      );
      return (response as any).data ?? response;
    ...
    onSuccess: (card: any, variables) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.flashcards.detail(card.id),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.decks.cards(card.deck_id),
      });
...
export function useDeleteCard() {
  ...
  return useMutation({
    mutationFn: async (cardId: number) => {
      const response =
        await FlashcardsService.deleteCardApiV1CardsCardIdDelete(cardId);
      return (response as any).data ?? response;
    },
    onSuccess: () => {
