...
const getDeckIcon = (deck: Deck) => {
  ...
  if (text.includes("code") || text.includes("dev") || text.includes("sql"))
    return Code;
...
export const DeckCard = React.memo(function DeckCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`h-[55%] relative bg-gradient-to-br ${gradientStyles[color]} flex items-center justify-center overflow-hidden`}
      >
        {/* Background Decor */}
        ...
        <div className="absolute top-3 right-3 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="bg-[#0f0f16] border-white/5 text-slate-200"
            >
              <DropdownMenuItem
                onClick={(e) => {
                  e.stopPropagation();
        ...
        <div className="relative z-10 flex flex-col items-center justify-center transform group-hover:scale-105 transition-transform duration-500">
          <div className="relative w-24 h-24 flex items-center justify-center">
            {/* Glow effect behind */}
            <div
              className={`absolute inset-0 rounded-full blur-xl opacity-40 ${iconColors[color].split(" ")[0].replace("text-", "bg-")}`}
            />
            ...
            <Icon
              size={32}
              className={`text-white drop-shadow-md relative z-10 ${iconColors[color].split(" ")[0]}`}
            />
          </div>
