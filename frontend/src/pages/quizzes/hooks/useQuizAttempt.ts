...
export function useQuizAttempt(quizId: number) {
  ...
  const { mutate: startAttempt } = useMutation({
    mutationFn: async () => {
      const response =
        await QuizzesService.startQuizAttemptApiV1QuizzesQuizIdStartPost(
          quizId,
        );
      return response;
    },
  ...
  const { mutate: submitQuiz, data: results } = useMutation({
    mutationFn: async () => {
      ...
      const answersArray: AnswerSubmit[] = Array.from(
      ...
      const response =
        await QuizzesService.submitQuizAttemptApiV1QuizzesAttemptsAttemptIdSubmitPost(
