...
import { SecurityBadge, GatekeeperLayout } from '@/components/auth/GatekeeperUI';
...
const GlitchText = ({ text }: { text: string }) => {
    return (
        <div className="relative inline-block group select-none">
            ...
            {/* Burn-in effect */}
            <span className="absolute inset-0 text-transparent stroke-2 stroke-foreground/10 pointer-events-none" style={{ WebkitTextStroke: '1px currentColor' }}>
                {text}
            </span>
        </div>
    );
            ...
            <div className="rounded-sm border border-destructive/30 bg-card overflow-hidden shadow-inner">
                <div className="flex items-center justify-between px-4 py-2 bg-muted/50 border-b border-border">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground">BLACKBOX_RECORDER</span>
                    </div>
                ...
                <div className="p-4 text-xs h-32 overflow-hidden flex flex-col justify-end relative">
                    ...
                    {lines.map((line, i) => (
                        ...
                        >
                            ...
                            <span className={line.includes("FAILED") || line.includes("CRITICAL") ? "text-destructive font-bold" : "text-muted-foreground"}>
                                {line}
                            </span>
                        </motion.div>
                    ))}
...
const IndustrialButton = ({ onClick, icon: Icon, label, variant = 'primary' }: any) => (
    <button
        onClick={onClick}
        className={`group relative w-full h-14 mt-3 overflow-hidden border transition-all active:translate-y-1 flex items-center justify-center gap-3 rounded-lg ${variant === 'primary'
    ...
    >
        ...
        <div className={`relative z-10 flex items-center gap-3 font-bold tracking-[0.25em] text-xs uppercase transition-colors ${variant === 'primary' ? 'text-foreground group-hover:text-primary' : 'text-muted-foreground group-hover:text-foreground'
            }`}>
            <Icon size={16} className={variant === 'primary' ? "text-destructive group-hover:text-primary transition-colors" : "opacity-50"} />
            {label}
        </div>
