...
// Styles
import './styles/synapse-theme.css';
...
export const ChatPage: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-screen bg-[var(--synapse-bg-primary)] text-[var(--synapse-text-primary)] overflow-hidden">
        ...
        {/* CENTER PANEL: CHAT */}
        <main className="flex-1 flex flex-col synapse-panel min-w-0 overflow-hidden">
        {/* Chat Header */}
        <div className="synapse-panel-header">
        <div className="flex items-center gap-2">
        <span className="synapse-panel-title">Chat</span>
        {/* Connection Indicator */}
        {isActiveSession && isConnected && (
            <div className="w-2 h-2 rounded-full bg-[var(--synapse-cyan)] synapse-pulse" />
        )}
        </div>
        ...
        {isLoading ? (
            <div className="synapse-empty-state h-full">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--synapse-cyan)]" />
            </div>
        ...
        {(sources.length > 0 || isActiveSession) && (
            <div className="p-4 border-t border-[var(--synapse-border-subtle)]">
            <ChatInput
            sessionId={numericSessionId}
        ...
        {/* Bottom Info Bar */}
        <div className="px-4 py-2 border-t border-[var(--synapse-border-subtle)] flex items-center justify-between">
        <div className="flex items-center gap-2">
        <span className="text-xs text-[var(--synapse-text-dim)]">
        {sources.length} source{sources.length !== 1 ? 's' : ''}
        </span>
        </div>
        <div className="text-xs text-[var(--synapse-text-dim)]">
        Synapse AI can be inaccurate, please double check its responses.
        </div>
        </div>
        </main>
