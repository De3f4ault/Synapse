...
export const useQuizzes = (params?: { page?: number; pageSize?: number }) => {
  return useQuery<QuizResponse[]>({
    ...
    queryFn: () =>
      QuizzesService.listQuizzesApiV1QuizzesGet(params?.page, params?.pageSize),
  });
};
...
export const useCreateQuiz = () => {
  ...
  return useMutation({
    mutationFn: (data: QuizCreate) =>
      QuizzesService.createQuizApiV1QuizzesPost(data),
    onSuccess: () => {
...
export const useSubmitQuizAttempt = () => {
  ...
  return useMutation<
    QuizResultResponse,
    Error,
    { attemptId: number; answers: AnswerSubmit[] }
  >({
    mutationFn: ({ attemptId, answers }) =>
      QuizzesService.submitQuizAttemptApiV1QuizzesAttemptsAttemptIdSubmitPost(
        attemptId,
        answers,
      ),
    onSuccess: () => {
