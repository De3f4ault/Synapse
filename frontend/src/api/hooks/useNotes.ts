...
const NOTE_KEYS = {
    ...
    search: (query: string, limit?: number) => [...NOTE_KEYS.all, 'search', query, limit] as const,
 ...
 */
export const useNotes = (params?: { parentId?: number | null; tags?: string; page?: number; pageSize?: number }) => {
    return useQuery<NoteResponse[]>({
        queryKey: NOTE_KEYS.list(params),
        queryFn: () => NotesService.listNotesApiV1NotesGet(params?.parentId ?? undefined, params?.tags, params?.page, params?.pageSize),
    });
};
...
export const useNoteTree = (rootId?: number | null) => {
    return useQuery<NoteTreeNode[]>({
        ...
        queryFn: () => NotesService.getNoteTreeApiV1NotesTreeGet(rootId ?? undefined),
    });
};
...
export const useNoteVersions = (noteId: number) => {
    return useQuery<NoteVersionResponse[]>({
        ...
        queryFn: () => NotesService.getNoteVersionsApiV1NotesNoteIdVersionsGet(noteId),
        enabled: !!noteId,
    });
};
...
export const useCreateNote = () => {
    ...
    return useMutation({
        mutationFn: (data: NoteCreate) => NotesService.createNoteApiV1NotesPost(data),
        onSuccess: () => {
...
export const useUpdateNote = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: NOTE_KEYS.detail(variables.noteId) });
            queryClient.invalidateQueries({ queryKey: NOTE_KEYS.versions(variables.noteId) });
            queryClient.invalidateQueries({ queryKey: NOTE_KEYS.lists() });
        },
    });
};
