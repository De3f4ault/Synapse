...
export const NeumorphicProgress = ({
    ...
    return (
        <div className={cn("w-full bg-black/40 rounded-full overflow-hidden shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)] border border-white/5", heights[size], className)}>
            <motion.div
                initial={{ width: 0 }}
...
export const NeumorphicBadge = ({
    ...
    const variants = {
        ...
        gradient: 'border-none bg-gradient-to-r from-white/10 to-transparent text-white',
    };
...
export const NeumorphicStats = ({
    ...
    return (
        <NeumorphicCard className={cn("p-4 flex flex-col justify-between h-full", className)}>
            ...
            <div>
                ...
                {trend && (
                    <div className={cn("text-xs mt-1", trendUp ? "text-emerald-400" : "text-rose-400")}>
                        {trend}
                    </div>
                )}
            </div>
