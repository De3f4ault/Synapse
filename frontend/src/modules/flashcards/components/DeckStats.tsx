...
import { BookOpen, TrendingUp, Clock, Target, Brain } from 'lucide-react';
...
function SimpleStatCard({ title, value, suffix = '', decimals = 0, icon: Icon, colorClass = 'text-primary', loading }: SimpleStatCardProps) {
    if (loading) {
        return (
            <Card className="p-4">
    ...
    return (
        <Card className="p-4">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <p className="text-2xl font-bold">{value.toFixed(decimals)}{suffix}</p>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <SimpleStatCard
                        ...
                        value={stats.due}
                        icon={Clock}
                        colorClass={stats.due > 0 ? 'text-orange-500' : 'text-green-500'}
                        loading={isLoading}
                    />
                    ...
                    <SimpleStatCard
                        ...
                        decimals={1}
                        icon={Target}
                        colorClass={stats.avgAccuracy >= 0.85 ? 'text-green-500' : stats.avgAccuracy >= 0.7 ? 'text-yellow-500' : 'text-red-500'}
                        loading={isLoading}
                    />
...
function calculateStats(cards: FlashcardResponse[]) {
    ...
    return cards.reduce(
        (acc, card) => {
            ...
            // Sum accuracy
            acc.totalAccuracy += card.accuracy ?? 0;
        ...
        },
        {
            ...
            avgAccuracy:
                cards.length > 0
                    ? cards.reduce((sum, card) => sum + (card.accuracy ?? 0), 0) / cards.length
                    : 0,
        }
    );
}
