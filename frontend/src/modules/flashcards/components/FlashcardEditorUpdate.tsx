...
export function FlashcardEditorUpdate({
    ...
    return (
        ...
        Image URL <span className="text-xs text-slate-500">(optional)</span>
        </Label>
        ...
        {frontText ? (
            <p className="text-lg whitespace-pre-wrap text-white">{frontText}</p>
        ) : (
        ...
        <div className="min-h-[200px] flex flex-col items-center justify-center text-center p-4">
        {backText ? (
            <p className="text-lg whitespace-pre-wrap text-white">{backText}</p>
        ) : (
            <p className="text-slate-500 italic">No text yet...</p>
        )}
        {backMediaUrl && !imageError.back && (
