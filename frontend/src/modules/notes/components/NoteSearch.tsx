...
function SearchResultCard({ result, query, onClick }: SearchResultCardProps) {
    ...
    const getContextSnippet = (content: string, query: string, maxLength = 150): string => {
    ...
    return (
        ...
        <Badge variant="outline" className={cn('text-xs', getMatchTypeColor(result.match_type))}>
        {result.match_type}
        </Badge>
        </div>
        </div>
        ...
        dangerouslySetInnerHTML={{
            __html: highlightText(getContextSnippet(result.content, query), query),
        }}
        />
        ...
        {result.score > 0 && (
            ...
            <TrendingUp className={cn('h-3 w-3', getRelevanceColor(result.score))} />
            <span className={cn('font-medium', getRelevanceColor(result.score))}>
