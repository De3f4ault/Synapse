...
import { BookOpen, Clock, AlertCircle, PlayCircle, RefreshCw } from 'lucide-react';
...
function DueItemCard({ item }: DueItemCardProps) {
    ...
    const getTypeColor = (type: string) => {
        if (type === 'flashcard') return 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950 dark:text-blue-400';
        if (type === 'quiz') return 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950 dark:text-purple-400';
    ...
    return (
        ...
        <Card className="hover:shadow-md transition-all border-l-4" style={{
            borderLeftColor: priority === 'high' ? 'rgb(239, 68, 68)' : priority === 'medium' ? 'rgb(234, 179, 8)' : 'rgb(34, 197, 94)'
        }}>
        ...
        {nextReview && (
            ...
            <span>
            Due {formatDistanceToNow(new Date(nextReview), { addSuffix: true })}
            </span>
            </div>
        )}
        ...
        {accuracy !== undefined && (
            ...
            className={cn(
                'text-xs',
                accuracy >= 0.8 ? 'bg-green-100 text-green-700' :
                accuracy >= 0.6 ? 'bg-yellow-100 text-yellow-700' :
        ...
        {item.data.times_reviewed !== undefined && (
            ...
            {item.data.times_reviewed} {item.data.times_reviewed === 1 ? 'review' : 'reviews'}
            </span>
        )}
        </div>
        </div>
