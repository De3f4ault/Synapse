...
import { Clock, Target, TrendingUp, CheckCircle2, XCircle, SkipForward, Flag } from 'lucide-react';
...
export function StudySession({
    ...
    const { startSession, completeSession, isStarting, isCompleting } = useStudySession({
        onComplete,
    });
    ...
    return (
        ...
        <Flag className={cn(
            'h-4 w-4',
            flaggedItems.has(currentIndex) && 'fill-yellow-500 text-yellow-500'
        )} />
        </Button>
        </div>
        </CardHeader>
        ...
        {currentItem.type === 'flashcard' ? (
            ...
            {!showingAnswer ? (
                ...
                {currentItem.data.front_text || currentItem.data.title || 'Flashcard Front'}
                </p>
                {currentItem.data.deck_name && (
                    <Badge variant="outline">{currentItem.data.deck_name}</Badge>
                )}
                <p className="text-sm text-muted-foreground">
                Press <kbd className="px-2 py-1 bg-muted rounded">Space</kbd> to reveal answer
                </p>
                </motion.div>
            ) : (
                // Back of flashcard
        ...
        <p>Keyboard: <kbd className="px-1 bg-muted rounded">Space</kbd> flip · <kbd className="px-1 bg-muted rounded">1</kbd> need practice · <kbd className="px-1 bg-muted rounded">2</kbd> got it · <kbd className="px-1 bg-muted rounded">F</kbd> flag</p>
        </div>
        </CardContent>
        </Card>
        </motion.div>
