...
import { Clock, AlertCircle, ChevronLeft, ChevronRight, Flag, Send } from 'lucide-react';
...
export function QuizTaker({ quiz, onComplete, onCancel }: QuizTakerProps) {
    ...
    const getTimeWarningLevel = (): 'normal' | 'warning' | 'critical' => {
        ...
        const percentRemaining = (timeRemaining / (quiz.time_limit_minutes * 60)) * 100;
    ...
    return (
        ...
        {timeRemaining !== null && (
            <motion.div
            animate={timeWarningLevel === 'critical' ? { scale: [1, 1.05, 1] } : {}}
            transition={{ repeat: Infinity, duration: 1 }}
            className={cn(
                'flex items-center gap-2 px-4 py-2 rounded-lg border-2 font-mono text-lg font-bold',
                timeWarningLevel === 'critical' && 'border-red-500 bg-red-50 text-red-600 dark:bg-red-950',
                timeWarningLevel === 'warning' && 'border-yellow-500 bg-yellow-50 text-yellow-600 dark:bg-yellow-950',
        ...
        className={cn(
            'h-4 w-4',
            flaggedQuestions.has(currentQuestion.id) && 'fill-yellow-500 text-yellow-500'
        )}
        />
        </Button>
        ...
        {attempt.questions.map((q, index) => {
            ...
            return (
                ...
                className={cn(
                    ...
                    isAnswered && !isCurrent && 'bg-green-50 border-green-300 hover:bg-green-100 dark:bg-green-950',
