...
export function ChunkExplorer({ documentId, className }: ChunkExplorerProps) {
    ...
    const { data: chunks, isLoading, isError } = useQuery({
        queryKey: ['documents', 'chunks', documentId, page, pageSize],
        queryFn: () => DocumentsService.getDocumentChunksApiV1DocumentsDocumentIdChunksGet(documentId, page, pageSize),
    });
