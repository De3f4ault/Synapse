...
import type { DocumentResponse, DeckResponse, NoteResponse } from '@/api/generated';
...
function ContextSection({ title, icon, children, defaultOpen = true, count }: ContextSectionProps) {
...
function getFileIcon(fileType: string) {
    ...
    if (fileType.includes('word')) return <FileType className="h-4 w-4 text-blue-500" />;
...
export function ContextPanel({
    ...
    const hasContext =
    document || relatedDecks.length > 0 || relatedNotes.length > 0 || weakAreas.length > 0;
